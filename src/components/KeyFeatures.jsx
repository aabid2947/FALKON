...
const KeyFeatures = () => {
  ...
  return (
    <section className="bg-black px-6 py-12 md:px-16 max-w-screen mx-auto">
      <h1 className="text-8xl font-bold text-gray-200 mb-8 text-center">Key Features</h1>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {features.map((feature, index) => {
          // Apply different transformations based on whether the card is in the left or right column
          const isLeftColumn = index % 2 === 0; // Even index for left column, odd index for right column
          const transformAmount = scrollY * 0.05; // Amount of movement based on scroll

          return (
            <div
              key={index}
              className="flex bg-gray-100 p-6 rounded-lg shadow-md"
              style={{
                transform: `translateX(${isLeftColumn ? -transformAmount : transformAmount}px)`,
